...
let updateToCommit = (repoReference, commitRef, sshPath, passphrase) => {
	return new Promise((resolve, reject) => {
		git.Commit.lookup(commitRef)
		.then((commit) => {
			return commit.getTree();
		})
		.then((commitTree) => {
			let checkoutOpts = {checkoutStrategy: (git.Checkout.STRATEGY.SAFE | git.Checkout.STRATEGY.RECREATE_MISSING)};
			return git.Checkout.tree(repoReference, commitTree, checkoutOpts);
		})
		.then(() => {
			return repoReference.setHeadDetached(commitRef);
		})
		.catch((err) => {
			reject(err);
		});
